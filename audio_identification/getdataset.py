...
def getdataset(music_array,rate):
    ...
    key_hashes = unique(fp[:, 1])  # Equivalent to keywords
    inverted_list = {k: [] for k in key_hashes}
    F1_time = array(fp[:, 0])  # Get t1 from fingerprint (Fingerprint 1)
    F1_hash = array(list(fp[:, 1]))  # Get t1 from hash (Fingerprint 1)
