...
for i in range(1,501):
  time1 = time.time()
  music = pydub.AudioSegment.from_file('./kikibouba_data/kiki/kiki_'  + f"{i:04d}" + '.wav',format="wav")
  ...
  if i % 50 == 0:
    np.savez_compressed('./last_result/kiki'+f"{i}", inverted_lists)
    np.savez_compressed('./last_result/kiki_ref'+f"{i}", inverted_lists_ref)
  print(i)
